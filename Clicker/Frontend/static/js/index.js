function GameSession() {
    ...
    this.update_rank = function (){
        ...
        getRank(this.styleLvl).then(rank => {
            audio.src = rank.music
            audio.volume = 0.2
            audio.autoplay = true
            const pic = document.getElementById('rank')
            pic.setAttribute('src', `${rank.image}`)
            pic.removeAttribute('opacity')
        })
        this.clicksRow = 0
        render()
    ...
    this.clear_rank = function (){
        this.styleLvl = 1
        this.clicksRow = 0
        const pic = document.getElementById('rank')
        pic.setAttribute('src', "Frontend/static/img/yamato.jpg")
        console.log("Ранг сброшен")
    }
}
...
function call_click() {
    ...
    click_animation(btn, 50)
    Game.clicksRow+=1
    rankChecker(Game.clicksRow, Game.styleLvl)

    if (Game.clicksRow > Game.styleLvl*5 && Game.styleLvl < 8) {
        Game.update_rank()
    }
...
function rankChecker(clicksRow, rank) {
    isRanking = true
    setTimeout(function() {
        if(Game.clicksRow <= clicksRow && Game.styleLvl === rank){
            Game.clear_rank()
        }
        isRanking = false
    }, 400*rank*0.55)
